...
## LibreSprite Android Mobile build

For Windows 7, use the GitHub Actions workflow in `.github/workflows/build-android-mobile.yml`.
It performs the native generation step on Ubuntu, downloads the official `ls-android-deps` tree,
and builds the `all_in_one` debug APK. This avoids requiring Android Studio on Windows 7.
